...
public class TelaEmprestimo implements DisplayEx {
	...
	public void alugaVeiculo() {
		...
		if (!ControladorPrincipal.getCtrlPrincipal().getCtrlFuncionario().validadeMatricula(numeroMatricula)
				|| !ControladorPrincipal.getCtrlPrincipal().getCtrlFuncionario().verificaFuncionarioExiste(numeroMatricula)) {
			ControladorPrincipal.getCtrlPrincipal().getCtrlLog().criaLog("Acesso Negado: Matricula invalida", numeroMatricula, null);
			ativo = false;
		}
		...
		while (ativo) {
			...
			if (ControladorPrincipal.getCtrlPrincipal().getCtrlFuncionario().verificaVeiculoPermitido(numeroMatricula)) {
				...
				if (!ControladorPrincipal.getCtrlPrincipal().getCtrlVeiculo().validadePlaca(placa)
					...
					ControladorPrincipal.getCtrlPrincipal().getCtrlLog().criaLog("Acesso Negado: Placa invalida", numeroMatricula, placa);
					ativo = false;
					break;
				}
				
				try {
					ControladorEmprestimo.getCtrlEmprestimo().verficaAdicionaEmprestimo(numeroMatricula, placa);
			...
			} else if (ControladorPrincipal.getCtrlPrincipal().getCtrlFuncionario().pegaPrimeiroVeiculo(numeroMatricula)
					.isDisponivel()) {
				...
				ControladorPrincipal.getCtrlPrincipal().getCtrlLog().criaLog("Acesso Permitido: Veiculo disponivel", numeroMatricula,
						ControladorPrincipal.getCtrlPrincipal().getCtrlFuncionario().pegaPrimeiroVeiculo(numeroMatricula).getPlaca());
				System.out.println("\nAcesso Permitido: Veiculo na lista e disponivel!");
				ativo = false;
				break;
			} else {
				ControladorPrincipal.getCtrlPrincipal().getCtrlLog().criaLog("Acesso Negado: Veiculo indisponivel", numeroMatricula,
						ControladorPrincipal.getCtrlPrincipal().getCtrlFuncionario().pegaPrimeiroVeiculo(numeroMatricula).getPlaca());
	...
	public void devolveVeiculo() {
		...
		while (ativo) {
			...
			if (ControladorEmprestimo.getCtrlEmprestimo().verificaValidadeEmprestimo(numeroMatricula, placa)) {
				...
				if (!ControladorPrincipal.getCtrlPrincipal().getCtrlVeiculo().validadeKm(quilometragemAtualS)) {
					ControladorPrincipal.getCtrlPrincipal().getCtrlLog().criaLog("Devolucao negada: Quilometragem invalida", numeroMatricula,
							placa);
					...
					try {
						ControladorEmprestimo.getCtrlEmprestimo().verificaEmp(quilometragemAtual, numeroMatricula,
								placa);
					} catch (Exception e) {
						System.out.println(e.getMessage());
					}
